...
- [ ] T001 Install `react-markdown` (`npm install react-markdown`).
...
- [ ] T002 Create `src/components/digests/DigestMarkdown.tsx`: a `react-markdown` wrapper with MD3-token-based component overrides (`p`, `strong`, `ul`, `li`), default configuration only (no raw-HTML plugins, per the spec's XSS mitigation).
- [ ] T003 [P] Create `src/components/digests/DigestSkeleton.tsx`: a loading placeholder matching the digest card's structure, using Tailwind's `animate-pulse` (no new dependency).
...
- [ ] T008 [US1] Render a friendly error message (no raw `errorMessage` exposed) when `status === 'failed'` (RF-4).
- [ ] T009 [US1] Render an explanatory empty state when the user has no digest at all yet (query resolved, no documents) (RF-5).
...
- [ ] T010 [US2] Create `src/app/history/page.tsx` wrapped in `ProtectedRoute` and rendering `AppHeader`, following the same pattern as `/dashboard`/`/settings` (RF-9).
- [ ] T011 [US2] Implement cursor-based pagination (`orderBy("createdAt", "desc")`, `limit(PAGE_SIZE)`, `startAfter(cursor)`) with a "Carregar mais" button that fetches the next page and appends to the list, disabled while loading or when there's no more data (RF-7, RF-8).
- [ ] T012 [US2] Render each list item with a formatted creation date (`Intl.DateTimeFormat`) and a truncated preview of `content.intro` via `DigestMarkdown` inside a `line-clamp-3` container (RF-10).
- [ ] T013 [US2] For list items where `status !== 'completed'`, render a status label ("Gerando..."/"Falhou") instead of attempting to render `content` (RF-11).
- [ ] T014 [US2] Add a `Link` to `/history` ("Histórico") in `src/components/AppHeader.tsx`'s navigation group, alongside Dashboard/Configurações/Sair (RF-12).
