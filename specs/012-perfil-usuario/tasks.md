...
- [x] T020 [P] After merge, write an ADR (`docs/adrs/0008-*.md`) documenting the account-deletion ordering pattern (digests → user doc → Auth account, stop on first failure) — primeira operação destrutiva/irreversível do projeto, per `plan.md`'s Constitution Check note.
