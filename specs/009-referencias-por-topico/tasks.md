...
- [x] T017 [US3] Manual validation: um tópico com referências mostra o accordion fechado por padrão e expande ao clicar; um tópico sem referências não mostra nenhuma caixa; um digest gerado antes desta spec (sem o campo `references`) continua renderizando normalmente. Confirmado ao vivo por Paulo em produção: a caixa "Referências (1)" apareceu fechada por padrão na seção que tinha referência real, e nenhuma caixa apareceu nas seções sem referência. Clique-pra-expandir e abertura em nova aba (RF-8) não foram testados explicitamente; cenário de digest pré-spec não foi checado à parte (mas o guard `?? []` é o mesmo padrão de baixo risco usado em toda a spec 008).
...
- [x] T020 [P] Manual end-to-end validation of the 5 acceptance scenarios in spec.md (RSS references, website-only reference, topic without any URL, expand interaction, old digest without the field). Scenarios 1-3 (data correctness) verified live via direct pipeline calls (T008/T013) and again via a real production cron run. Scenario 4 (accordion closed by default) confirmed live by Paulo. Scenario 5 (old digest without the field) not explicitly re-checked after merge, low risk per T017's note.
- [x] T021 [P] After merge, write ADR 0007 (`docs/adrs/0007-*.md`) documenting the structured URL-attributed aggregation + citation-restricted AI prompt + defensive server-side reference filtering pattern introduced in this spec, per `plan.md`'s Constitution Check note. Also folded in the content-fidelity fix (fix/ai-content-fidelity, PR #18) found live right after this spec shipped — same underlying principle (prompt instructions alone aren't a guarantee; code-level checks are), so documented together rather than as a separate ADR.
