...
- Fluxo de login com Google e por e-mail/senha (cadastro, entrada e redefinição de senha), com a criação do perfil do usuário no Firestore feita por um endpoint autenticado (`POST /api/auth/profile`, Firebase Admin SDK) independente do provedor, não mais por escrita direta do client. O e-mail de redefinição de senha é enviado pelo próprio Firebase Authentication — domínios institucionais/corporativos com filtro de e-mail agressivo podem bloquear ou colocar em spam mensagens automatizadas desse tipo; sem solução do lado do app pra isso
...
- Pipeline de geração automática de digests (`GET /api/cron/generate`, protegido por `CRON_SECRET`, agendado a cada hora via GitHub Actions — veja "Disparo do Cron em produção"): agrega fontes RSS/website do usuário, gera um resumo detalhado por tópico via GPT-4o-mini (com até 3 referências reais por tópico, nunca inventadas — filtradas em código contra as URLs de origem, não só por instrução ao modelo) e persiste em `users/{uid}/digests`, com isolamento de falhas por fonte/usuário e proteção contra reprocessamento duplicado. Fontes `website` são validadas por metadados de SEO no `<head>` antes de serem usadas, e contribuem até 3 links de seção da navegação (ex.: "Esportes", "Política") como referências reais, além da página inicial — não mais só a URL raiz do site
- `/dashboard` exibe o digest mais recente em tempo real (skeleton enquanto `processing`, mensagem amigável em `failed`, estado vazio para quem ainda não tem nenhum), com as referências de cada tópico numa caixa colapsável fechada por padrão e a data real de geração no título; `/history` lista os digests anteriores com paginação — ambos renderizando o texto gerado pela IA via `react-markdown`
- Opção "Manter conectado" em `/login` (marcada por padrão), controlando se a sessão sobrevive a fechar o navegador (`browserLocalPersistence`) ou termina com ele (`browserSessionPersistence`)
- Página `/profile` para editar o nome de exibição (via `updateProfile` do Firebase Auth, refletido automaticamente em toda a UI), cadastrar links de redes sociais pessoais (Twitter/X, Instagram, LinkedIn) e excluir a conta permanentemente (perfil, preferências e histórico de digests), atrás de uma confirmação digitada — via os endpoints autenticados `POST`/`DELETE /api/profile`
